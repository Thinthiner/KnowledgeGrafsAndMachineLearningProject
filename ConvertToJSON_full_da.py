...
$ python3 ConvertToJSON_full2.py Words_test.txt
...
sparql =  """
    SELECT ?lexeme ?representation ?lexcatLabel ?lemma {
       ?form ontolex:representation "TO_REPLACE"@da .
       ?lexeme ontolex:lexicalForm / ontolex:representation "TO_REPLACE" @da ; 
...
for i in range(1, len(full_data)):
    for j in range(len(full_data[i])):
        ...
        while not success:
            try:
                words = full_data[i][j].split()
                for w in words:
                    m = {}
                    m['word'] = w
                    m['lexeme'] = get_sparql_dataframe(endpoint, sparql.replace("TO_REPLACE", w))
                    ...
                    if not m['lexeme']:
                        ...
                        m['entity'].append(str(set_entity))
                        set_entity += 1
                        m['verbs'].append('None')
                    data.append(m)
                success = True
